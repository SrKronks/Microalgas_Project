...
class TensorFlowSequenceModel(ForecastModel):
    ...
    def __init__(self, name: str, cell: str, epochs: int = 80, lags: int = 4) -> None:
        super().__init__()
        self.name = name
        self.cell = cell
        self.epochs = epochs
    ...
    def fit_predict(self, train: pd.Series, horizon: int, **_: object) -> np.ndarray:
        ...
        if self.cell == "lstm":
            model.add(tf.keras.layers.LSTM(16, input_shape=(self.lags, 1)))
        elif self.cell == "bilstm":
            model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(16), input_shape=(self.lags, 1)))
        elif self.cell == "gru":
            model.add(tf.keras.layers.GRU(16, input_shape=(self.lags, 1)))
        elif self.cell == "cnn":
            model.add(tf.keras.layers.Conv1D(16, kernel_size=2, activation="relu", input_shape=(self.lags, 1)))
            model.add(tf.keras.layers.Flatten())
        else:
            model.add(tf.keras.layers.SimpleRNN(16, input_shape=(self.lags, 1)))
        ...
        self.metadata = {"lags": self.lags, "epochs": self.epochs, "cell": self.cell}
        return np.asarray(preds, dtype=float)


def get_deep_learning_models(random_state: int = 42, heavy: bool = False) -> list[ForecastModel]:
        ...
        models.append(
            RecursiveSklearnModel(
                ...
                lambda: make_pipeline(
                    StandardScaler(),
                    MLPRegressor(hidden_layer_sizes=(32, 16), max_iter=1000, random_state=random_state),
                ),
                lags=4,
            )
        )
    ...
    for name, cell in sequence_names:
        models.append(TensorFlowSequenceModel(name, cell, epochs=120 if heavy else 40))
